# Note: Dispersal and movement parameters can be adjusted to run over a shorter sequence within 0 to 1 in 0.1 increments, to reduce run time. 
...
num_rows <- length(x_a_grid) * length(x_b_grid) * timesteps * length(seq(0, 1, by = 0.1)) * length(seq(0, 1, by = 0.1)) * length(seq(0, 1, by = 0.1)) * length(seq(0, 1, by = 0.1)) 
num_cols <- 13  
...
# Cooperative Scenario (Sole Owner Scenario)
...
# Calculate the number of iterations over M_ba, M_ab, D_ba and D_ab
